...
void FmsMessageListener::setPublisher(rclcpp_lifecycle::LifecycleNode::SharedPtr node) {
  ...
  if (node) {
    ...
    m_rosPublisher = node->create_publisher<std_msgs::msg::String>("/rics/fms_to_robot", qos);
    m_publisherSet = true;
...
void FmsMessageListener::OnRecvMsgHandle(const string& topic, const string& payload) {
  ...
  if (m_publisherSet && m_rosPublisher) {
    // 创建 ROS 2 消息并发布
    auto ros_msg = make_shared<std_msgs::msg::String>();
    // 构建包含主题和 payload 的 JSON 消息
    ros_msg->data = "{\"topic\": \"" + topic + "\", \"message\": \"" + payload + "\"}";
...
std::vector<std::string> FmsMessageListener::GetSubscribeTopics() {
  return {"command/" + m_sn + "/login-ack", "command/" + m_sn + "/lock",
...
void FmsMessageListener::CreatePublisher() {
  if (m_rosNode) {
    // 配置 QoS 并创建发布者
    const rclcpp::QoS& qos = rclcpp::SystemDefaultsQoS();
    m_rosPublisher = m_rosNode->create_publisher<std_msgs::msg::String>("/rics/fms_to_robot", qos);
